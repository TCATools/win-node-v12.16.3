...
for (const [name, definition] of aria_query_1.roles) {
...
for (const [element, names] of aria_query_1.elementRoles) {
    if ([...names].some((name) => interactiveRoles.has(name))) {
        interactiveElements.push(element);
    }
}
function isInteractiveElement(node) {
    const elementType = getElementType_1.default(node);
    if (!aria_query_1.dom.has(elementType)) {
        return false;
    }
    return interactiveElements.some((elementRole) => matchesElementRole_1.default(node, elementRole));
}
