...
function isCaptionsTrackElement(node) {
    const kind = node.type === "VElement" && utils_1.getElementAttributeValue(node, "kind");
    return kind && typeof kind === "string" && kind.toLowerCase() === "captions";
}
const rule = {
    meta: {
        docs: {
            url: utils_1.makeDocsURL("media-has-caption")
        },
        messages: {
        ...
        return utils_1.defineTemplateBodyVisitor(context, {
            VElement(node) {
                ...
                const mediaElementTypes = audio
                    ...
                    .concat("audio", "video");
                if (!mediaElementTypes.includes(utils_1.getElementType(node))) {
                    return;
                }
                const muted = utils_1.getElementAttribute(node, "muted");
                if (muted && (utils_1.getAttributeValue(muted) || "").toString() !== "false") {
                    return;
                }
                ...
                const trackElements = node.children.filter((child) => child.type === "VElement" &&
                    trackElementTypes.includes(utils_1.getElementType(child)));
