...
const rule = {
    meta: {
        docs: {
            url: utils_1.makeDocsURL("aria-unsupported-elements")
        },
        messages: {
    ...
    create(context) {
        return utils_1.defineTemplateBodyVisitor(context, {
            VElement(node) {
                if (!(aria_query_1.dom.get(utils_1.getElementType(node)) || {}).reserved) {
                    return;
                }
                node.startTag.attributes.forEach((attribute) => {
                    const name = utils_1.getAttributeName(attribute);
