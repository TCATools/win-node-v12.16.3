...
function hasAttributes(node, names) {
    return names.every((name) => utils_1.getElementAttribute(node, name) !== null);
}
...
const rule = {
    meta: {
        docs: {
            url: utils_1.makeDocsURL("role-has-required-aria-props")
        },
        messages: {
    ...
    create(context) {
        return utils_1.defineTemplateBodyVisitor(context, {
            VElement(node) {
                const elementType = utils_1.getElementType(node);
                if (!aria_query_1.dom.get(elementType)) {
                    return;
                }
                const roleValue = utils_1.getElementAttributeValue(node, "role");
