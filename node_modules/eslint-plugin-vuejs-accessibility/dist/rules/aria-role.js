...
const rule = {
    meta: {
        docs: {
            url: utils_1.makeDocsURL("aria-role")
        },
        messages: {
    ...
    create(context) {
        return utils_1.defineTemplateBodyVisitor(context, {
            VAttribute(node) {
                ...
                if (ignoreNonDOM && !aria_query_1.dom.has(utils_1.getElementType(node.parent.parent))) {
                    return;
                }
                if (!utils_1.isAttribute(node, "role")) {
                    return;
                }
                const value = utils_1.getAttributeValue(node);
