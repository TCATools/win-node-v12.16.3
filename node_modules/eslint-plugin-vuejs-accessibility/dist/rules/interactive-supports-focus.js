...
for (const [role, definition] of aria_query_1.roles) {
...
function isDisabledElement(node) {
    return (utils_1.getElementAttributeValue(node, "disabled") ||
        (utils_1.getElementAttributeValue(node, "aria-disabled") || "").toString() ===
            "true");
}
...
function hasTabIndex(node) {
    const attribute = utils_1.getElementAttribute(node, "tabindex");
    if (!attribute) {
        return false;
    }
    const value = utils_1.getAttributeValue(attribute);
...
const rule = {
    meta: {
        docs: {
            url: utils_1.makeDocsURL("interactive-supports-focus")
        },
        messages: {
    ...
    create(context) {
        return utils_1.defineTemplateBodyVisitor(context, {
            VElement(node) {
                const role = utils_1.getElementAttributeValue(node, "role");
                if (aria_query_1.dom.has(utils_1.getElementType(node)) &&
                    utils_1.hasOnDirectives(node, interactiveHandlers) &&
                    !hasTabIndex(node) &&
                    !isDisabledElement(node) &&
                    !utils_1.isHiddenFromScreenReader(node) &&
                    !utils_1.isPresentationRole(node) &&
                    isInteractiveRole(role) &&
                    !utils_1.isInteractiveElement(node)) {
