...
const rule = {
    meta: {
        docs: {
            url: utils_1.makeDocsURL("no-onchange")
        },
        messages: {
    ...
    create(context) {
        return utils_1.defineTemplateBodyVisitor(context, {
            VElement(node) {
                if (!["select", "option"].includes(utils_1.getElementType(node))) {
                    return;
                }
                if (utils_1.hasOnDirective(node, "change") && !utils_1.hasOnDirective(node, "blur")) {
